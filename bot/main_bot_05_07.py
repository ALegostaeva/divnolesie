...
def save_participants_to_xlsx(filename='участники.xlsx'):
    ...
    for participant in participants:
        ...
        won_seasons = participant.get('won_seasons', [])
    ...
    if data['type'] == 'confirmation':
        ...
        # Получить команды, доступные админам
            ...
            if user and user.get('admin'):
                admin_message = (
                            ...
                            "- выбрать победителей - Позволит выбрать участников, которые успещно завершили сезон и смогут начать следующий сезон автоматически без оплаты и подтверждения. ВАЖНО! Определить победителей ДО команды старта нового сезона.\n"
        ...
        # Переместить на соту
            ...
            if user and user.get('admin'):
                ...
                logger.info(f"Запрос профиля: {selected_user_id}, {next_cell}")
        ...
        # Чужой профиль для админа
            ...
            if user and user.get('admin'):
                ...
                if selected_user:
                    ...
                    user_info_message = (f"Участник: {selected_user['first_name']} {selected_user['last_name']},\n"
        ...
        # Выбор единорога у пользователя, чтобы удалить
            ...
            if selected_user:
                ...
                if has_more:
                    unicorns_keyboard['buttons'].append([{"action": {"type":"callback", "label":"Ещё","payload":json.dumps({"action": "more", "selected_user_id": selected_user_id, "list": "unicorns_delete", "page": page+1})}}])
        ...
        # добавить пользователю сезон как успешно завершенный
            ...
            if user.get('admin', False):
                ...
                user_message = f"Поздравляем! Вы успешно завершили текущий сезон! Вы примите участие в разыгрыше Дивнолесных даров! А так же соверщенно бесплатно сможете продолжить свое путешествие в следующем сезоне! Очень ждем от вас новых отчетов! Удачи, дорогой путник!"
            else:
                message = "У вас нет прав для выполнения этой команды."
