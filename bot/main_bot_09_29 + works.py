...
def mark_event_as_processed(event_id):
    ...
    logger.info(f"Сохраняем событие {event_id} как обработанное.")
    save_logs(logs)
...
def update_user(vk_id, first_name=None, last_name=None, is_participant=None, current_cell=None, coins=None, kurator=None, exp_egg=None, exp_boloto=None, exp_snow=None, exp_les=None, exp_kamen=None, exp_pustyn=None, won_season=None, notifications=None):
    users = load_users()
    info_marathon = load_info_marathon()
    current_season = str(info_marathon.get('current_season', "1"))
...
def send_notification_to_all_users(message, keyboard=None):
    ...
    changes_made = False
    for user in users:
        logger.info("sent mes get user: %s", user['vk_id'])
...
def notify_admins(vk_id, first_name = "", last_name = "", user_exists = ""):
    ...
    for admin in admins:
        ...
        keyboard = {
            "inline": True,
            "buttons": [
                [{"action": {"type": "callback", "label": "Принять", "payload": json.dumps({"action": "accept", "user_id": vk_id, "name": user_name})}},
                {"action": {"type": "callback", "label": "Отклонить", "payload": json.dumps({"action": "reject", "user_id": vk_id, "name": user_name})}}]
            ]
        }
    ...
    if data['type'] == 'confirmation':
            ...
            if user_info_vk:
                ...
                keyboard = {
                        ...
                        "buttons": [
                            ...
                            [{"action": {"type": "text", "label": "участвую"}},{"action": {"type": "text", "label": "правила"}}],
                            [{"action": {"type": "text", "label": "отключить уведомления"}},{"action": {"type": "text", "label": "включить уведомления"}}],
        ...
        # заявка на участие в марафоне
            ...
            if user:
                if user.get('is_participant', True):
                    user_message = "Вы уже участвуете в марафоне в этом сезоне."
                else:
                    info_marathon = load_info_marathon()
        ...
        #FAQ - частые вопросы
            ...
            faq_keyboard = {
                    "inline": True,
                    "buttons": [
                        [{"action": {"type": "callback", "label": "Как растить единорога", "payload": json.dumps({"action": "faq", "question": "unicorn"})}}],
                        [{"action": {"type": "callback", "label": "Артефакты", "payload": json.dumps({"action": "faq", "question": "artifacts"})}},{"action": {"type": "callback", "label": "Квесты", "payload": json.dumps({"action": "faq", "question": "quests"})}}],
                        [{"action": {"type": "callback", "label": "Телепорты и их отличия", "payload": json.dumps({"action": "faq", "question": "teleports"})}}],
                        [{"action": {"type": "callback", "label": "Как двигаться по карте", "payload": json.dumps({"action": "faq", "question": "movement"})}}],
                        [{"action": {"type": "callback", "label": "Как писать отчет", "payload": json.dumps({"action": "faq", "question": "review"})}}],
        ...
        # Получить команды, доступные админам
            ...
            if user and user.get('admin'):
                admin_message = (
                            ...
                            "- profile [ID_пользователя или Фамилия] - Покажет полный профиль пользователя, со всей имеющейся информацией. Например: profile 123456.\n"
                            ...
                            "ID пользователя можно взять из команды -участники- или команды id [фамилия]. Вот еще доступные команды:"
                        )
        ...
        # Сброс участников марафона перед стартом сезона
            ...
            if user and user.get('admin'):
                ...
                users_notification = "В нашем Дивнолесье стартовал новый сезон! Ждем тебя в гости! Подавай заявку и спеши скорее на встречу приключениям!"
                users_notification_keyboard = {
                        "inline": True,
                        "buttons": [
        ...
        # Чужой профиль для админа
            ...
            if user and user.get('admin'):
                ...
                if not selected_user_id.isdigit():
                    selected_user = next((u for u in users if u['last_name'] == selected_user_id or u['first_name'] == selected_user_id), None)
                else:
                    selected_user = next((u for u in users if u['vk_id'] == int(selected_user_id)), None)
        ...
        # Любой другой случай
            ...
            keyboard = {
                    ...
                    "buttons": [
                        ...
                        },{"action": {"type": "text", "label": "правила"}}]
                    ]
                }
            if user:
                user_name = first_name + ' ' + last_name
        ...
        # частые вопросы
            ...
            # Справки для часто задаваемых вопросов
            faq_responses = {
                "unicorn": "КАК РАСТИТЬ ЕДИНОРОГА\n\nВ Залесье обитают дикие единороги. Но их, к сожалению, невозможно поймать и одомашнить. Выход есть! Взрастим единорога сами! Пройдя 6 уникальных* сот, вы получите единорожье яйцо той территории, на которой вы находитесь. То есть в ваших силах получить яйцо каменного города, пустыни, болот, снежного перевала или междумирья. А дальше взращиваем приобретенное яйцо с помощью игровых единиц опыта! Напоминаем, что 1 единица опыта=400 страниц текста. 15 единиц опыта - вот вам и единорог 2ой стадии (и, как следствие, возможность брать не только первую, но и вторую тему соты). Хочешь третью тему в соте? Взрасти единорога до 3тьей (взрослой) стадии! И будет тебе счастье!\n\nПодробнее об игровом опыте здесь: https://divnolesie.ru/pages/rules.html#expirience).\nПодробнее про питомцев Дивнолесья читай здесь: https://divnolesie.ru/pages/rules.html#pet\n\n* уникальная сота - та сота, на которую вы не ступали ранее. ",
                "artifacts": "АРТЕФАКТЫ\n\nАртефакты — это уникальные предметы, которые дают различные игровые бонусы. Как совершается покупка артефактов: \n1.Путник приходит на клетку с торговыми путями.\n2. Путник готовит необходимое количество юникоинов.\n3.Путник в своем отчете пишет о том, какой артефакт он покупает.\nАртефакты можно использовать сразу же или оставить до востребования в рюкзаке (статистике), воспользовавшись ими позднее при первой надобности. \nСписок пртефактовв и их описание смотри здесь: https://divnolesie.ru/pages/artefacts.html. \nПодробнее про покупку и применение артефактов здесь - https://divnolesie.ru/pages/rules.html#artefact.",
                "movement": "КАК ДВИГАТЬСЯ ПО КАРТЕ\n\nНачальная сота, с которой начинается путешествие путника, назначается ответственным куратором. Так происходит лишь при старте. Со второй соты путник выстраивает свой маршрут по сотам самостоятельно, исходя из понравившихся тем в соте. В случае, если у путника нет в наличии артефактов и редких питомцев, следующая сота должна прилегать к предыдущей, т.е. ребра сот должны соприкасаться.\nПодробнее про то, как играть - смотри здесь: https://divnolesie.ru/pages/rules.html#how-to-play.\nПравила оформления отчета доступны по ссылке здесь: https://divnolesie.ru/pages/rules.html#review-rules",
                "review": "КАК ПИСАТЬ ОТЧЕТ\n\nЧто нужно указать в отчете: \n\nТэги. В конце отзыва ставим тэг #ДивноеЗалесье (А/К), где буква в скобках обозначает куратора (Аня - А, Ксюша - К).\n\nПрисказка. Это короткое описание, которое прислал вам Куратор, когда вы ступили на соту.\n\nТекущая сота. Пример: C5(1). В скобках указывается номер темы.\n\nСледующий ход. В отчете должна быть указана следующая сота, куда делается ход. Если участник хочет воспользоваться второй, а не первой темой, то в посте он прописывает свое желание. Например, С5(2). Указание соты без номера темы автоматически приравнивается к тому, что участник читает первую из предложенных тем.\n\nОбъем произведения. Указывается объем прочитанной бумажной книги. Если книга была прочитана в электронной версии, то возможны два варианта: при наличии этой книги на Литрес - указывать объем страниц этой книги с сайта. Или не указывать страницы вообще. В этом случае сота будет зачтена, но ни опыт, ни монетки юникоин начислены не будут.\n\nАртефакт. Уведомить о покупке или использовании артефакта (опционально).\n\nОтзыв о книге. Развернутый отзыв о книге, ваши впечатления, эмоции, что понравилось, а что не нашло отклик в вашем сердце. В отзыв нельзя включать аннотацию к книге. Общее количество символов в отзыве: не менее 1000 (проверить можно здесь simvoli.net). \n\nПодробнее ты можешь прочитать здесь: https://divnolesie.ru/pages/review-rules",
                "quests": "КВЕСТЫ \n\nКвесты бывают разные: постоянные и временные (ограниченный срок действия). И квестов очень много. За каждый квест дается особенная награда. Так, например, есть квестовые задания, даже не предусматривающие прочтение книг! И да, за них дается игровая плюшка. А есть квесты, чтение книг по которым принесет удвоение награды! Таким образом можно взрастить единорога намного быстрее. \nВсе квесты ты найдешь здесь: https://vk.com/topic-155728712_41277103.",
                "teleports": "ТЕЛЕПОРТЫ И ИХ ОТЛИЧИЯ\n\nЗалесье - большое. А как быть, если хочется оказаться воооон там на карте? Конечно, воспользоваться возможностью переместиться!\n\nА как это сделать?\n\n На данный момент в Залесье существуют аж несколько видов телепортов. Давайте разбираться чем они друг от друга отличаются и как их взять в арсенал к использованию.\n\n1) Дойти до сундучков на карте, выполнить задание соты и переместиться. Сундуки расположены в каждой из местностей, поэтому проблем добраться хотя бы до одного из них - не должно быть. Перемещение разовое, далее вы шествуете с той соты, на которую приземлились после перемещения. Сота должна соприкасаться с вашей ребром, конечно.\n\n2) Дойти до квестовой зоны и взять квест 'Включаем кино'. Квест подразумевает просмотр любой экранизации и отчета по ней во вторниковой рубрике. Перемещение разовое, далее вы стартуете опять же с перемещенной соты по своему желанию.\n\n3) Катапульта. Да-да, вы вполне можете катапультироваться на нужную соту. Катапульт в Залесье аж три: 7в (катапульта), 13m (хищный цветок), 22b (гейзер). Все три катапульты обладают способностью перемещать участника в пределах одной земли и за нее. Конечно, переместиться вы сможете не просто так, а купив артефакт. Активирующий артефакт можно приобрести также на торговых путях. Катапульты стоят: 25 и 40 коинов.\n\n4) Гонец (значок свитка с пером)! Сота 'гонец' - это многоходовая клетка (как казино), в ней нужно прочесть не менее 3 книг по рандомному выбору куратора. Есть и финальное задание - не связано с прочтением книги, так называемое 'околокнижное задание'. И вот как раз оно дарует участнику перемещение на любую соту в текущей местности.\n\n5) Телепорт-артефакт. Купленный за коины (25 юникоинов) телепорт может отнести путника только к любой доске квестов. Не на любую соту вообще карты, а только на квест. Потом - после выполнения квеста или квестов - возвращаешься на прежнюю соту. Исключения - если ты покупаешь артефакт-телепорт, выполняешь квест-кино и перемещаешься на любую соту -тогда без возвращения.\n\n6) Сота С24, именуемая тролльей тропой/тролльим мостом. Выполнив все 6 заданий и выиграв книгу из 'Эксклюзивной классики', вы также перемещаетесь на рандомную соту из текующей местности.",
                "allrules": "ПРАВИЛА ИГРЫ\n\nВсе правила игры ты найдешь здесь: https://divnolesie.ru/pages/rules.html. \nНо не пугайся, друг! По всем вопросам можно обратиться к своему куратору. Пусть не забывает, чей он раб на игровой период, не правда ли? И все-таки сначала прочти основную информацию, это поможет тебе включиться в игру быстрее!"
            }
        ...
        # добавить пользователю сезон как успешно завершенный
            ...
            if user.get('admin', False):
                ...
                user_message = f"Поздравляем! Вы успешно завершили текущий сезон! Вы примите участие в розыгрыше Дивнолесных даров! А также совершенно бесплатно сможете продолжить свое путешествие в следующем сезоне! Очень ждем от вас новых отчетов! Удачи, дорогой путник!"
            else:
                message = "У вас нет прав для выполнения этой команды."
...
def send_message(peer_id, message, keyboard = None, event_id = None):
    ...
    try:
        ...
        if 'error' in result:
            ...
            logger.info(f"Нет ошибки при отправки {peer_id} с event_id {event_id}")
